...
export const getPropertiesWithinViewPort = async (
  minLat: number,
  minLng: number,
  maxLat: number,
  maxLng: number
  ...
  const response = await instance.get(
    `properties/search/map?minLat=${minLat}&minLng=${minLng}&maxLat=${maxLat}&maxLng=${maxLng}`
  );
  return response.data;
};

export const getAmenitiesWithinViewPort = async (
  minLat: number,
  minLng: number,
  maxLat: number,
  maxLng: number,
  ...
  const response = await instance.get(
    `recommendation/amenities/search/map?minLat=${minLat}&minLng=${minLng}&maxLat=${maxLat}&maxLng=${maxLng}&category=${category}&limit=${limit}`
  );
  return response.data;
};
...
export const predictPropertyPrice = async (data: {
  ...
  const response = await instance.post(
    "recommendation/prediction/property/price",
    data
  );
  return response.data;
};
...
export const getRecommendedProperties = async (
  ...
  if (user_id) {
    response = await instance.get(
      `recommendation/home?lat=${lat}&lng=${lng}&limit=${limit}&radius_km=${radius_km}&user_id=${user_id}`
    );
  } else {
    response = await instance.get(
      `recommendation/home?lat=${lat}&lng=${lng}&limit=${limit}&radius_km=${radius_km}`
    );
  }
  return response.data;
};

export const getRecommendedPropertiesDetail = async (
  propertyId: number,
  limit: number = 6,
  radius_km: number = 20
) => {
  const response = await instance.get(
    `recommendation/related/${propertyId}?limit=${limit}&radius_km=${radius_km}`
  );
  return response.data;
};
...
export const getLivabilityScore = async (data: { propertyIds: number[] }) => {
  const response = await instance.post(
    "recommendation/livability/scores/batch",
    data
  );
  return response.data;
};

export const getDistrictNewsInBounds = async (
  minLat: number,
  minLng: number,
  maxLat: number,
  maxLng: number
) => {
  const response = await instance.get("recommendation/news/map", {
