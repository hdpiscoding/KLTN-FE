...
export const UserSidebar: React.FC = () => {
    ...
    return (
        <>
            {/* Desktop Sidebar */}
            <aside
                className="hidden sm:flex w-64 bg-white border-r border-gray-200 h-screen sticky top-0 flex-col shadow-sm">
                {/* Logo */}
                ...
                {/* Navigation Menu */}
                <nav
                    className="flex-1 p-4 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
                    {menuItems.map((item) => (
                        <SidebarItem
                            key={item.path}
                            icon={item.icon}
