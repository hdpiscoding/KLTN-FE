...
export const AmenitiesWithClustering: React.FC<AmenitiesWithClusteringProps> = ({
    amenities,
    selectedAmenity,
    onAmenitySelect,
    currentZoom,
    ...
    const [markersByCategory, setMarkersByCategory] = useState<{
        [K in AmenityCategory]?: {[key: string]: google.maps.marker.AdvancedMarkerElement}
    }>({});
    // Separate clusterer for each category
    const clusterersByCategory = useRef<{[K in AmenityCategory]?: MarkerClusterer}>({});

    // Don't render if zoom too low
    const shouldShowMarkers = currentZoom >= minZoomForMarkers;

    // Filter amenities based on filter state
    const filteredAmenities = amenities.filter(amenity => filterState[amenity.category]);
    ...
    useEffect(() => {
        ...
        Object.entries(markersByCategory).forEach(([category, markers]) => {
            const clusterer = clusterersByCategory.current[category as AmenityCategory];
            if (clusterer && markers) {
                const markersList = Object.values(markers);
                if (markersList.length > 0) {
                    clusterer.addMarkers(markersList);
                    console.log(`📍 Clustered ${markersList.length} ${category} amenity markers`);
                }
            }
        });
    }, [markersByCategory]);
